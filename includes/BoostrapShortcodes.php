...
class BoostrapShortcodes {
  ...
  /**
   * Main BoostrapShortcodes Instance
   *
   * Ensures only one instance of BoostrapShortcodes is loaded or can be loaded
   *
   * @static
   * @return BoostrapShortcodes - Main instance
   */
  ...
  static function bootstrap_shortcodes_popper_script()  {
    ...
    if ( has_shortcode( $post->post_content, 'popover') || has_shortcode( $post->post_content, 'tooltip') ) {
      wp_enqueue_scripts( 'popper', 'https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.8/dist/umd/popper.min.js' );
      // wp_enqueue_scripts( 'popper' );
    }
  }

  static function register_shortcodes() {
    wp_enqueue_style( 'bs-callout', plugins_url( 'css/bs-callout.css' , __FILE__ ));
...
/*--------------------------------------------------------------------------------------
  ...
  *-------------------------------------------------------------------------------------*/
  static function container( $atts, $content = null ) {
    ...
    $class  = ( $atts['xclass'] )   ? ' ' . $atts['xclass'] : 'container-fluid';
  ...
  *-------------------------------------------------------------------------------------*/
  static function modal( $atts, $content = null ) {
    ...
    add_action('wp_footer', function() use ($modal_output) {
      // echo '<pre> modal in wp_footer </pre><br>';
      echo $modal_output;
    }, 100,0);
...
/*--------------------------------------------------------------------------------------
  ...
  *-------------------------------------------------------------------------------------*/
  static function tabs( $atts, $content = null ) {
    ...
    $atts_map = self::bs_attribute_map( $content );
  ...
  *-------------------------------------------------------------------------------------*/
  ...
  //
  // ======================================================================== // 
  static function bs_attribute_map($content, $att = null) {
      $res = array();
      $return = array();
  ...
  // ======================================================================== // 
  static function bs_cleanup_domdocument($content) {
  ...
  // ======================================================================== // 
  static function bs_fix_shortcodes($content){
    $array = array (
      '<p>[' => '[', 
      ']</p>' => ']', 
      ']<br />' => ']',
